...
const checkDogs = function(dogsJulia, dogsKate) {
    ...
    dogsJuliaKate.forEach((age, index) => {
        console.log(dogsJuliaKate[index]);
        console.log(age >= 3 ? `Dog number ${index+1} is an adult, and is ${age}` : 
            `Dog number ${index+1} is still a puppy`);
    });
};

checkDogs([3, 5, 2, 12, 7], [4, 1, 15, 8, 3]);
console.log(`separator` + `-`.repeat(300));
